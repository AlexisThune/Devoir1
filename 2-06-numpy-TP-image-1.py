...
# %%
# votre code
tb = 255*np.ones(shape = (91, 91, 3), dtype = np.uint8)
tb[:, :, :] = [255, 255, 0]
tb[5::10, :, :] = [0, 0, 255]
tb [:, 5::10, :] = [0, 0, 255]
print(tb[0, 0])
...
# votre code
im = plt.imread('les-mines.jpg')
v = np.empty((im.shape[0], im.shape[1], 4), dtype = im.dtype)
v[:, :, :3] = im
v[:, :, 3] = 128
plt.imshow(v)
...
im = plt.imread('les-mines.jpg')
p = im.copy()
p = np.ndarray.astype(p, dtype = float)/255
o=np.mean(p, 2)
i = (0.299*p[:, :, 0]+0.587*p[:, :, 1]+0.114*p[:, :, 2])*255
ima = i.astype(np.uint8)
c = np.square(p)
r = np.sqrt(o)
...
axes[0, 0].imshow(o, cmap = 'gray')
axes[1, 1].imshow(o, cmap = 'gray')
axes[2, 2].imshow(o, cmap = 'gray')
axes[1, 0].imshow(i, cmap = 'gray')
axes[0, 2].imshow(i, cmap = 'gray')
axes[2, 1].imshow(i, cmap = 'gray')
